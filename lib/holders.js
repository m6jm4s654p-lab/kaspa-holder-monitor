...
const KASPATROL_URL='https://kaspatrol.com/addresses';
...
async function fetchKaspatrolAddresses(){
  const r=await fetch(KASPATROL_URL,{cache:'no-store',headers:BROWSER_HEADERS});
  if(!r.ok){
    const error=new Error(`Kaspatrol ${r.status}`);
...
export async function fetchHolderSnapshot(){
  try{
    const text=plain(await fetchKaspatrolAddresses());

...
function MarketChart({candles,prices,volumes,lang,t}){
 ...
 return <div className="marketWrap">
  <div className="chartMode"><span className="realDot"/> <b>4H {hasCandles?t.ohlc:t.priceLive}</b><span className="chartRange">{t.last60}</span><em>{t.realData}</em></div>
  ...
  <svg className="fixedPriceAxis" viewBox={`0 0 ${AX} ${H}`} aria-label={t.priceScale}>
...
export default function Dashboard(){
 ...
 useEffect(()=>{const browserLang=navigator.language.toLowerCase(),detected=browserLang.startsWith('ja')?'ja':browserLang.startsWith('ko')?'ko':browserLang.startsWith('zh')?'zh':browserLang.startsWith('es')?'es':'en',saved=localStorage.getItem('khm-lang'),l=T[saved]?saved:detected;setLang(l);document.documentElement.lang=l==='zh'?'zh-CN':l;if('serviceWorker'in navigator)navigator.serviceWorker.register('/sw.js').catch(()=>{});Promise.allSettled([fetch('/api/holders').then(r=>{if(!r.ok)throw new Error(`holders ${r.status}`);return r.json()}),fetch('/api/history?days=120',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(`history ${r.status}`);return r.json()})]).then(([h,his])=>{if(h.status==='fulfilled')setHolder(h.value);if(his.status==='fulfilled')setHistory(his.value)});},[]);
 useEffect(()=>{
  let active=true;
 ...
 const allDaily=useMemo(()=>dailyRows(history.rows||[]),[history.rows]);const rows=useMemo(()=>allDaily.slice(-Math.max(holderPeriod,2)),[allDaily,holderPeriod]);
 ...
 return <main className="shell">
  <header className="topbar"><div className="brandBlock"><img src="/kaspa-logo.svg" className="kaspaMark" alt="Kaspa"/><div><div className="kaspaWord">KASPA</div><div className="monitorWord">HOLDER MONITOR</div></div></div><div className="techbitBrand"><span>ANALYTICS BY</span><b>@TechBit</b></div><select className="lang" value={lang} onChange={changeLang} aria-label="Language"><option value="ja">日本語</option><option value="en">English</option><option value="ko">한국어</option><option value="zh">中文</option><option value="es">Español</option></select></header>
  <section className="hero"><div className="live"><i/> {t.live}</div><h1>{t.tag}</h1><p>Small Steps. A Bigger KASPA.</p></section>
  <section className="card priceCard"><div className="row"><div><div className="eyebrow">{t.price} (USD)</div><div className="price">{price?.usd?`$${price.usd.toFixed(price.usd<.1?5:3)}`:'—'}</div><div className={price?.change24h>=0?'up':'down'}>{price?.change24h!=null?pct(price.change24h):'—'} <span>24h</span></div></div><div className="fixedTimeframe"><b>4H</b><span>{t.view60}</span></div></div><MarketChart candles={allCandles} prices={allPrices} volumes={allVolumes} lang={lang} t={t}/><div className="marketStats"><div><span>{t.change60}</span><b className={periodChange>=0?'up':'down'}>{pct(periodChange)}</b></div><div><span>{t.high}</span><b>{periodHigh?`$${periodHigh.toFixed(5)}`:'—'}</b></div><div><span>{t.low}</span><b>{periodLow?`$${periodLow.toFixed(5)}`:'—'}</b></div><div><span>{t.volume}</span><b>{compactUsd(latestVol)}</b></div></div><div className="priceSource"><span className="realDot"/> {t.priceLive} · 4H OHLC · {t.realData}</div><div className={`updateStatus ${updateDelayed?'delayed':''}`}><span>{t.priceStatus} {timeFmt(priceUpdatedAt)}</span><span>{t.chartStatus} {timeFmt(chartUpdatedAt)}</span><b>{updateDelayed?t.updateDelayed:t.autoRefresh}</b></div></section>
  ...
  <div className="statsGrid supplyGrid"><section className="card stat"><div className="eyebrow">{t.supply}</div><b>{holder?.circulating?`${(holder.circulating/1e9).toFixed(2)} B KAS`:'—'}</b><div className="supplyBar"><i style={{width:'100%'}}/></div><small>{t.circulating}</small></section><section className="card stat whaleCard"><div className="eyebrow">{t.whales}</div><div className="whaleHero"><b>{top100.toFixed(2)}%</b><span>{t.heldTop100}</span></div><div className="whaleStack">{segments.map(s=><i key={s.label} className={s.cls} style={{width:`${s.val}%`}} title={`${s.label}: ${s.val.toFixed(2)}%`}/>)}</div><div className="whaleLegend">{segments.map(s=><span key={s.label}><i className={s.cls}/>{s.label}<b>{s.val.toFixed(2)}%</b></span>)}</div><p className="whaleHelp">{t.whaleHelp}</p></section></div>
  ...
  <section className="card dailyCard"><div className="chartHead"><b>{t.daily}</b><span>100K+</span></div><BarChart values={daily} t={t}/></section>
  <section className="card compareCard"><div className="chartHead"><b>{t.compare}</b><span>{holderPeriod}D</span></div><NormalizedCompare holderValues={h100} priceValues={priceSeries} t={t}/></section>
  ...
  <footer><div className="footerBrand"><img src="/kaspa-logo.svg" alt="Kaspa"/><div><b>@TechBit</b><span>KASPA ON-CHAIN ANALYTICS</span></div></div><p>{t.foot}</p><p>{t.source}: Kaspatrol / api.kaspa.org · CoinGecko<br/>{t.updated}: {holder?.capturedAt?new Date(holder.capturedAt).toLocaleString(LOCALES[lang]):'—'}</p></footer>
</main>
}

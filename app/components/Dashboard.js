...
export default function Dashboard(){
 const [lang,setLang]=useState('ja'),[holder,setHolder]=useState(null),[price,setPrice]=useState(null),[priceHistory,setPriceHistory]=useState({prices:[],volumes:[],candles:[],maPrices:[]}),[history,setHistory]=useState({enabled:false,rows:[]}),[holderPeriod,setHolderPeriod]=useState(30),[scoreOpen,setScoreOpen]=useState(false),[priceUpdatedAt,setPriceUpdatedAt]=useState(null),[chartUpdatedAt,setChartUpdatedAt]=useState(null),[priceDelayed,setPriceDelayed]=useState(false),[chartDelayed,setChartDelayed]=useState(false);const t=T[lang];

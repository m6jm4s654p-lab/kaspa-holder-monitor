...
export default function Dashboard(){
 ...
 return <main className="shell">
  ...
  <div className="statsGrid"><section className="card stat"><div className="eyebrow">{t.addresses} (1+ KAS)</div><b>{fmt(holder?.total1Plus)}</b><small>{holder?.fallback?'Fallback snapshot':'Network-wide'}</small></section><section className="card stat score"><div className="eyebrow scoreTitle">{t.holderScore}<button onClick={()=>setScoreOpen(v=>!v)}>?</button></div>{score==null?<div className="scoreMissing">—<small>{t.insufficient}</small></div>:<><div className="gauge" style={{'--score':`${score*3.6}deg`}}><b>{score}</b></div><small className="signalPill">{signal}</small></>}{scoreOpen&&<div className="scoreExplain"><b>{t.scoreBreakdown}</b><p>{t.scoreHelp}</p>{scoreParts&&<div className="scoreParts">{[[t.breadth,scoreParts.breadth,'35%'],[t.large,scoreParts.large,'25%'],[t.concentration,scoreParts.concentration,'20%'],[t.divergence,scoreParts.divergence,'20%']].map(([l,v,w])=><div key={l}><span>{l} <em>{w}</em></span><b>{v}</b></div>)}</div>}<small>{t.scoreNote}</small></div>}</section></div>
  ...
  <section className="card"><div className="sectionTitle">{t.tiers}</div><div className="table head"><span>Tier</span><span>{t.current}</span><span>{t.d7}</span><span>{t.d30}</span></div>{tiers.map(key=>{const c7=nearestChange(allDaily,key,7),c30=nearestChange(allDaily,key,30);return <div className={`table ${key==='100K+'?'hot':''}`} key={key}><span>{key}</span><b>{fmt(holder?.cumulative?.[key])}</b><span className={c7?.abs>=0?'up':'down'}>{c7&&c7.daysObserved>=5?`${c7.abs>=0?'+':''}${fmt(c7.abs)}`:'—'}</span><span className={c30?.abs>=0?'up':'down'}>{c30&&c30.daysObserved>=25?`${c30.abs>=0?'+':''}${fmt(c30.abs)}`:'—'}</span></div>})}</section>

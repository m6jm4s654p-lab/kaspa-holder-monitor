...
function MarketChart({candles,prices,volumes,lang,t}){
 ...
 return <div className="marketWrap">
  ...
  <div className="maLegend" aria-label="Moving averages">
...
export default function Dashboard(){
 const [lang,setLang]=useState('ja'),[holder,setHolder]=useState(null),[price,setPrice]=useState(null),[priceHistory,setPriceHistory]=useState({prices:[],volumes:[],candles:[],maPrices:[]}),[derivatives,setDerivatives]=useState(null),[history,setHistory]=useState({enabled:false,rows:[]}),[holderPeriod,setHolderPeriod]=useState(30),[scoreOpen,setScoreOpen]=useState(false),[priceUpdatedAt,setPriceUpdatedAt]=useState(null),[chartUpdatedAt,setChartUpdatedAt]=useState(null),[priceDelayed,setPriceDelayed]=useState(false),[chartDelayed,setChartDelayed]=useState(false);const t=T[lang];
 ...
 return <main className="shell">
  ...
  <section className={`card pulseCard ${pulse.tone}`}>
   ...
   <div className="pulseFoot"><span>{pulseText.notice}</span><b>{lang==='ja'?'Bybit · KASUSDT 無期限':'Bybit · KASUSDT Perpetual'}</b></div>
  </section>

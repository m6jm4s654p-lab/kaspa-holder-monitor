...
function BarChart({values,t}){if(values.length<1)return <div className="emptyChart">{t.noHistoryYet}</div>;const W=420,H=120,mid=60,max=Math.max(...values.map(v=>Math.abs(v)),1);return <svg className="chart" viewBox={`0 0 ${W} ${H}`}><line x1="0" y1={mid} x2={W} y2={mid} stroke="#ffffff22"/>{values.map((v,i)=>{const bw=W/values.length*.55,scaled=Math.abs(v)/max*52,h=v===0?2:scaled,x=i*W/values.length+(W/values.length-bw)/2,y=v>0?mid-h:v<0?mid:mid-1;return <rect key={i} x={x} y={y} width={bw} height={h} rx="1.5" fill={v>=0?'#49eac1':'#ff6b6b'}/>})}</svg>}
...
export default function Dashboard(){
 ...
 const h100=rows.map(r=>r.a_100k_plus).filter(Number.isFinite),priceSeries=rows.map(r=>r.kas_price_usd).filter(Number.isFinite),daily30Rows=allDaily.slice(-31),daily30Values=daily30Rows.map(r=>r.a_100k_plus==null?null:Number(r.a_100k_plus)),daily=daily30Values.slice(1).map((v,i)=>Number.isFinite(v)&&Number.isFinite(daily30Values[i])?v-daily30Values[i]:null).filter(Number.isFinite);
 ...
 return <main className="shell">
  <header className="topbar"><div className="brandBlock"><img src="/kaspa-logo.svg" className="kaspaMark" alt="Kaspa"/><div><div className="kaspaWord">KASPA</div><div className="monitorWord">HOLDER MONITOR</div></div></div><div className="headerTools"><span className="versionBadge">v2.2.13</span><select className="lang" value={lang} onChange={changeLang} aria-label="Language"><option value="ja">日本語</option><option value="en">English</option><option value="ko">한국어</option><option value="zh">中文</option><option value="es">Español</option></select><img className="headerTechbitLogo" src="/techbit-logo.png" alt="TechBit · Crypto × Data × Future"/></div></header>
